...
type RepairPdfToolProps = {
  tool: ToolDefinition
}

export function RepairPdfTool({ tool }: RepairPdfToolProps) {
  ...
  return (
    <BaseToolLayout
      ...
      onProcess={async (files, context) => {
        if (!window.api?.repairPdf) {
          throw new Error('The PDF repair engine is not available in this build.')
        }
        ...
        const result = await window.api.repairPdf({
          inputPaths,
          outputDir,
        })
        ...
        context.setResult(
          <>
            <Badge className='border-0 bg-accent/15 text-accent'>Repair attempted</Badge>
      ...
      options={
        <div className='space-y-4 text-sm'>
          <p className='text-xs text-muted'>
            Attempts to fix corrupted PDFs by restructuring internal objects and rebuilding the cross-reference table.
          </p>
