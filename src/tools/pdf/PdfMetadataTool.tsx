...
type PdfMetadataToolProps = {
  tool: ToolDefinition
}

export function PdfMetadataTool({ tool }: PdfMetadataToolProps) {
  const { preferences, setDefaultOutputDir } = useAppStore()
  const [title, setTitle] = useState('')
  const [author, setAuthor] = useState('')
  const [subject, setSubject] = useState('')
  const [keywords, setKeywords] = useState('')
  const [creator, setCreator] = useState('DocFlow Pro')
  ...
  return (
    <BaseToolLayout
      ...
      onProcess={async (files, context) => {
        if (!window.api?.metadataPdf) {
          throw new Error('The metadata engine is not available in this build.')
        }
        ...
        const result = await window.api.metadataPdf({
          inputPaths,
          outputDir,
          metadata: {
        ...
        context.setResult(
          <>
            <Badge className='border-0 bg-accent/15 text-accent'>Metadata updated</Badge>
      ...
      options={
        <div className='space-y-4 text-sm'>
          ...
          <div className='space-y-2'>
            <div className='text-xs font-semibold uppercase text-muted'>Author</div>
            <Input value={author} onChange={(event) => setAuthor(event.target.value)} />
          </div>
          <div className='space-y-2'>
            <div className='text-xs font-semibold uppercase text-muted'>Subject</div>
            <Input value={subject} onChange={(event) => setSubject(event.target.value)} />
          </div>
          <div className='space-y-2'>
            <div className='text-xs font-semibold uppercase text-muted'>Keywords</div>
            <Input value={keywords} onChange={(event) => setKeywords(event.target.value)} />
          </div>
          <div className='space-y-2'>
            <div className='text-xs font-semibold uppercase text-muted'>Creator</div>
