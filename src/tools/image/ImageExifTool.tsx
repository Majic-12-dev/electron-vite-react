...
type ImageExifToolProps = {
  tool: ToolDefinition
}

export function ImageExifTool({ tool }: ImageExifToolProps) {
  ...
  return (
    <BaseToolLayout
      ...
      onProcess={async (files, context) => {
        if (!window.api?.stripExif) {
          throw new Error('The EXIF stripping engine is not available in this build.')
        }
        ...
        const result = await window.api.stripExif({
          inputPaths,
          outputDir,
        })
        ...
        context.setResult(
          <>
            <Badge className='border-0 bg-accent/15 text-accent'>Metadata removed</Badge>
      ...
      options={
        <div className='space-y-4 text-sm'>
          <p className='text-xs text-muted'>
            Removes GPS, camera model, and other EXIF metadata without altering visible pixels.
          </p>
